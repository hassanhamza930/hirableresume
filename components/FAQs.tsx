...
// No need for question mark icon import
  ...
  return (
    <div className="w-full flex flex-col justify-start items-center px-[5%] sm:px-[10%] mt-24 md:mt-48">
      ...
      <BlurReveal
        ...
        className="mb-2 text-2xl sm:text-3xl md:text-4xl text-center flex flex-wrap gap-x-2 justify-center items-center text-shadow-2xs text-shadow-blue-600 px-4 w-full md:w-[450px]"
      ...
      <BlurReveal
        ...
        text="Find answers to common questions about our AI resume builder and how it can help you land more interviews."
      />
    ...
    >
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value={`item-${faq.id}`} className="border-none">
          {/* Custom styling for the accordion trigger and chevron */}
          <AccordionTrigger className={cn(
            "px-5 py-4 hover:no-underline group",
            "[&[data-state=open]>svg]:rotate-180 [&[data-state=open]>svg]:text-white",
            "[&>svg]:text-white/50 [&>svg]:size-5 [&>svg]:transition-all [&>svg]:duration-300"
          )}>
